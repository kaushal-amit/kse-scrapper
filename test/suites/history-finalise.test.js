...
(async()=>{
  ...
  const {rows}=await db.query("select * from tradingview_history where symbol='HFA'");
  const b=rows[0];
  ...
  ck('change_value computed', Number(b.change_value)===19, b.change_value);
  ck('change_pct computed', Number(b.change_pct)===19, b.change_pct);
  ...
  const {rows:rows2}=await db.query("select count(*)::int c, max(close_price) cp, max(high_price) hp from tradingview_history where symbol='HFA'");
